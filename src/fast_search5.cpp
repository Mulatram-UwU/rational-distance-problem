...
//
// Build: g++ -O3 -march=native -fopenmp -std=c++17 -o bin/fast_search5.exe src/fast_search5.cpp
// Run:   ./bin/fast_search5.exe --N 20000000000
//        ./bin/fast_search5.exe --N 100000000 --no-mod3      (v3's filter set)
//
// --seglo <k> / --seghi <k> restrict the run to the segment indices [k, k')
// (default: all of [0, nseg)).  C is handled independently of the segment it
// falls in and every counter is a sum over C, so running the segments in
// several pieces and adding the printed lines reproduces the single whole run
// number for number.  That is what makes a multi-hour run resumable: a machine
// that goes down mid-piece costs only that piece.  `partners`, `candidates`,
// `tested`, `solutions`, `divisor_steps` and `sieve_overflow` are additive,
// `max_partners_per_C` is a maximum; `N` and the filter options must be the
...
struct Worker {
    ...
    u64 cSteps, cLeaves, cPartners, cCand, cTest, cSol, maxPart, cSieveOnly;
    ...
    void go(int idx, u64 u, u64 v) {
        ...
        if (idx == nf) { ++cLeaves; leafAdd(u, v); return; }
        u64 p = pr[idx];
        int e2 = 2 * ex[idx];
        u64 ucur = u;
        for (int f = 0; f <= e2; ++f) {
...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        else if (!strcmp(argv[i], "--seglo") && i + 1 < argc) segLo = strtoull(argv[++i], 0, 10);
        else if (!strcmp(argv[i], "--seghi") && i + 1 < argc) segHi = strtoull(argv[++i], 0, 10);
    ...
    // the pieces of a chunked run: [segLo, segHi) intersected with [0, nseg)
    const u64 sHi = segHi < nseg ? segHi : nseg;
    const u64 sLo = segLo < sHi ? segLo : sHi;
    if (sLo != 0 || sHi != nseg)
        fprintf(stderr, "[range] segments [%llu,%llu) of %llu\n",
                (unsigned long long)sLo, (unsigned long long)sHi,
                (unsigned long long)nseg);
    std::atomic<u64> doneSeg(0);
    u64 gSteps = 0, gLeaves = 0, gPartners = 0, gCand = 0, gTest = 0, gSol = 0, gMaxPart = 0, gOver = 0;
...
#pragma omp parallel
    {
        ...
        W.cSieveOnly = 0;
        W.cSteps = W.cLeaves = W.cPartners = W.cCand = W.cTest = W.cSol = 0; W.maxPart = 0;
        W.lst.reserve(1 << 14);
...
#pragma omp for schedule(dynamic, 1)
        for (long long si = (long long)sLo; si < (long long)sHi; ++si) {
            ...
            for (size_t i = 0; i < len; ++i) {
                ...
                // visit the primes largest-first: the complement product that the
                // v <= 3N prune looks at then grows as early as possible
                for (int a = 0, b = W.nf - 1; a < b; ++a, --b) {
                    u64 tp = W.pr[a]; W.pr[a] = W.pr[b]; W.pr[b] = tp;
                    int  te = W.ex[a]; W.ex[a] = W.ex[b]; W.ex[b] = te;
                }
                W.fillPowers();

#ifdef PROBE_NO_WALK
                W.lst.clear();
                ++W.cSieveOnly; continue;
#else
                W.lst.clear();
                W.go(0, 1, 1);
...
#endif
                {
                    u64 *L = W.lst.data();
                    size_t k = W.lst.size();
                    if (k <= 32) {                       // insertion sort, the common case
                        for (size_t a = 1; a < k; ++a) {
                            u64 t = L[a];
                            size_t b = a;
                            while (b > 0 && L[b - 1] > t) { L[b] = L[b - 1]; --b; }
                            L[b] = t;
                        }
                    } else {
                        std::vector<u64> tmpv(L, L + k);
                        std::sort(tmpv.begin(), tmpv.end());
                        for (size_t q = 0; q < k; ++q) L[q] = tmpv[q];
                    }
                }
                if (dumpC) {
            ...
            if (d % 1000 == 0) {
                if (sLo == 0 && sHi == nseg) {
                    ...
                    fprintf(stderr, "[prog] %llu/%llu segments (of segments [%llu,%llu))\n",
                            (unsigned long long)d, (unsigned long long)(sHi - sLo),
                            (unsigned long long)sLo, (unsigned long long)sHi);
                }
            }
        }
...
#pragma omp critical
        {
            gSteps += W.cSteps; gLeaves += W.cLeaves; gPartners += W.cPartners; gCand += W.cCand;
